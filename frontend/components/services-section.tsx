...
export function ServicesSection() {
  ...
  const t = (key: string): string => {
    const translations: Record<string, Record<string, string>> = {
      ...
      "section.subtitle": {
        "zh-Hans": "百千成电子具备深度整合硬件研发、PCB设计、DFM可制造性设计、元器件选型、SMT贴片加工、PCBA代工代料、整机组装与测试交付等全链路能力，帮助客户将产品从概念验证快速推进到稳定量产。在产品研发与设计阶段，我们提供电路设计/原理图设计、PCB布局、高可靠性设计、EMC/EMI与热设计优化，并以DFM/DFA导向的工程评审来降低量产风险，为储能BMS/PCS、工业控制板等复杂系统奠定可靠基础。进入NPI试产与量产阶段，依托全自动智能化SMT贴片产线实现微米级贴装与焊接，支持0.4mm间距BGA、高频通信模块等精密工艺，日产能可达500万点，良品率长期稳定在99.8%+。同时，我们提供从PCBA到整机的一站式OEM交付：涵盖插件/手焊、整机组装、三防漆涂覆、ICT在线测试、功能测试（FCT）及完整的包装出货体系，并执行72小时老化测试与多维度功能验证，确保医疗设备、智能机器人、汽车电子等高可靠产品实现高一致性、低缺陷率的规模化交付。",
        "en": "Baiqiancheng Electronics possesses comprehensive capabilities for integrating hardware research and development, PCB design, DFM (Design for Manufacturability) engineering design, component selection, SMT (Surface Mount Technology) assembly, PCBA (Printed Circuit Board Assembly) outsourcing and material supply, complete assembly and testing delivery. This enables customers to rapidly advance their products from proof of concept to stable mass production.During the product development and design stage, we offer circuit design/ schematic design, PCB layout, high-reliability design, EMC/EMI and thermal design optimization, and conduct engineering reviews guided by DFM/DFA to reduce production risks. This lays a reliable foundation for complex systems such as energy storage BMS/PCS and industrial control boards. In the NPI (New Product Introduction) trial production and mass production stage, we utilize fully automated intelligent SMT (Surface Mount Technology) assembly lines for micrometer-level assembly and welding, supporting precise processes such as 0.4mm pitch BGA and high-frequency communication modules. The daily production capacity can reach 5 million points, with a long-term stable yield rate of over 99.8%.Additionally, we provide one-stop OEM (Original Equipment Manufacturer) delivery from PCBA to complete machinery: covering component insertion/hand soldering, complete assembly, anti-corrosion paint coating, ICT (In-Circuit Testing) online testing, functional testing (FCT), and complete packaging and shipping systems. We also conduct 72-hour aging tests and multi-dimensional functional verification to ensure high consistency and low defect rate in the mass production of high-reliability products such as medical equipment, intelligent robots, and automotive electronics.",
      },
