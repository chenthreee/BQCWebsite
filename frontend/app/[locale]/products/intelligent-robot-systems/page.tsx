...
async function fetchIntelligentRobotData(locale: string) {
  const categoryQuery = `
...
export default async function IntelligentRobotSystemsPage({
  params,
}: {
  ...
  const { categoryInfo, products } = await fetchIntelligentRobotData(locale)

  return (
    <PageLayout
      title={
        categoryInfo?.title ||
        (locale === "en" ? "Intelligent Robot Systems" : "智能机器人系统")
      }
      subtitle={
        categoryInfo?.description ||
        (locale === "en"
          ? "Advanced Intelligent Robot Control Systems"
          : "先进的智能机器人控制系统")
      }
      breadcrumbs={[
