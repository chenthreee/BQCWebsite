...
export default function IndustrialControlBoardsPage() {
  ...
  const fetchCategoryInfo = async () => {
    const query = `
      query GetIndustrialControlBoardsCategory($locale: I18NLocaleCode) {
        productCategories(
          locale: $locale,
          filters: { name: { eq: "industrial-control-boards" } }
        ) {
          title
          description
        }
  ...
  const fetchProducts = async () => {
    const query = `
      query GetIndustrialControlBoardsProducts($locale: I18NLocaleCode) {
        products(
          locale: $locale,
          filters: {
            category: { name: { eq: "industrial-control-boards" } }
          },
          sort: "order:asc"
        ) {
          documentId
    ...
    try {
      console.log('🔍 Fetching Industrial Control Boards products for locale:', locale)
      const res = await fetch(GRAPHQL_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ query, variables: { locale } })
      ...
      console.log('✅ Industrial Control Boards Products loaded:', data?.products?.length || 0)
    ...
    return (
      <PageLayout
        ...
        breadcrumbs={[
          ...
          { label: locale === "en" ? "Industrial Control Boards" : "工业控制板", href: `/${locale}/products/industrial-control-boards.html` },
        ]}
        backgroundImage="/images/products/industrialBoardBreadcrumb.png"
      >
  ...
  return (
    <PageLayout
      title={categoryInfo?.title || (locale === "en" ? "Industrial Control Boards" : "工业控制板")}
      subtitle={categoryInfo?.description || (locale === "en" ? "Reliable and Stable Industrial Control Boards" : "可靠稳定的工业控制板卡")}
      breadcrumbs={[
        { label: locale === "en" ? "Products Center" : "产品中心", href: `/${locale}/products.html` },
        { label: categoryInfo?.title || (locale === "en" ? "Industrial Control Boards" : "工业控制板"), href: `/${locale}/products/industrial-control-boards.html` },
      ]}
        backgroundImage="/images/products/industrialBoardBreadcrumb.png"
    >
      {/* 产品列表 */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
        {products.map((product: any) => (
          <Link key={product.documentId} href={`/${locale}/products/industrial-control-boards/${product.slug}.html`}>
          <div key={product.documentId} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col h-full">
            ...
            <div className="p-6">
              ...
              <Link
                href={`/${locale}/products/industrial-control-boards/${product.slug}.html`}
