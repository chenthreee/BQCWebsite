...
export default function EnergyStoragePcsPage() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      console.log('🔍 Fetching PCS products for locale:', locale)
      const res = await fetch(GRAPHQL_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ query, variables: { locale } })
      ...
      console.log('✅ PCS Products loaded:', data?.products?.length || 0)
      setSubCategories(data?.products || [])
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <PageLayout
        title={locale === "en" ? "Energy Storage PCS" : "储能PCS"}
        subtitle={locale === "en" ? "Loading..." : "加载中..."}
        breadcrumbs={[
          { label: locale === "en" ? "Products Center" : "产品中心", href: `/${locale}/products.html` },
          { label: locale === "en" ? "Energy Storage PCS" : "储能PCS", href: `/${locale}/products/energy-storage-pcs.html` },
        ]}
  ...
  return (
    <PageLayout
      ...
      subtitle={categoryInfo?.description || (locale === "en" ? "High-performance Energy Storage Converter System" : "高性能的储能变流器系统")}
      breadcrumbs={[
        { label: locale === "en" ? "Products Center" : "产品中心", href: `/${locale}/products.html` },
        { label: categoryInfo?.title || (locale === "en" ? "Energy Storage PCS" : "储能PCS"), href: `/${locale}/products/energy-storage-pcs.html` },
      ]}
      backgroundImage="/images/products/PCSBreadcrumb.png"
    >
      {/* PCS产品列表 */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
        {subCategories.map((product: any) => (
          <Link key={product.documentId} href={`/${locale}/products/energy-storage-pcs/${product.slug}.html`}>
