...
async function fetchPowerStorageData(locale: string) {
  const productsQuery = `
    query GetPowerStorageProducts($locale: I18NLocaleCode) {
      products(
        ...
        filters: { 
          ...
          sub_category: { name: { eq: "power-storage" } }
        },
        sort: "order:asc"
      ) {
        documentId
...
export default async function PowerStorageBmsPage({
  ...
  return (
    <PageLayout
      title={categoryInfo?.title || (locale === "en" ? "Power Storage BMS" : "电力储能BMS")}
      subtitle={categoryInfo?.description || (locale === "en"
        ? "High-reliability Power Storage Battery Management System"
        : "高可靠性电力储能电池管理系统")}
      breadcrumbs={[
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
        {products.map((product: any) => (
          <Link
            ...
            href={`/${locale}/products/energy-storage-bms/power-storage/${product.slug}.html`}
          >
