...
export default function EnergyStorageEmsPage() {
  ...
  const fetchProducts = async () => {
    const query = `
      query GetEnergyStorageEmsProducts($locale: I18NLocaleCode) {
        products(
          locale: $locale,
          filters: { 
            category: { name: { eq: "energy-storage-bms" } },
            sub_category: { name: { eq: "energy-storage-ems" } }
          },
          sort: "order:asc"
        ) {
          documentId
  ...
  const fetchCategory = async () => {
    const query = `
      query GetEnergyStorageEmsCategory($locale: I18NLocaleCode) {
        productSubCategories(
          locale: $locale,
          filters: { name: { eq: "energy-storage-ems" } }
        ) {
          title
          description
        }
  ...
  if (loading) {
    return (
      <PageLayout
        ...
        backgroundImage="/images/products/EMSBreadcrumb.png"
      >
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
        {products.map((product: any) => (
          <Link key={product.documentId} href={`/${locale}/products/energy-storage-bms/energy-storage-ems/${product.slug}.html`}>
          <div key={product.documentId} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col h-full ">
            ...
            <div className="p-6">
              ...
              <Link
                href={`/${locale}/products/energy-storage-bms/energy-storage-ems/${product.slug}.html`}
